...
class CustomProgressBar(ttk.Canvas):
    def __init__(self, master=None, width=300, height=30, **kwargs):
        ...
        self.handle = self.create_oval(
            0, 0, self.handle_radius * 2, self.handle_radius * 2, fill=self.handle_color
        )
    ...
    def update_progress_bar(self, event=None):
        ...
        self.coords(
            self.handle,
            handle_center - self.handle_radius,
            0,
            handle_center + self.handle_radius,
            self.height,
        )